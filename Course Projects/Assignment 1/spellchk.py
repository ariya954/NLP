...
def select_correction(typo, predict):
    # pick the candidate most similar to the typo; break ties by model score
    best = max(predict, key=lambda p: (difflib.SequenceMatcher(None, typo.lower(), p['token_str'].lower()).ratio(), p['score']))
    return best['token_str']
